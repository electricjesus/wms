...
mysql_select_db($database_dbGlobal, $dbGlobal);
	...
	elseif (isset($id))
	{
		...
		// 1 RP, 2 RB, 3 PB, 4 AS
		$accttypestr_a = array('Panel','City Bkg','Prov Bkg','Arvd Stocks','City Bkg + VAT','Prov Bkg + VAT','Tie-up','Credit Memo/Warehouse BO','Panel BO Report','Panel Inventory Report','DSR');
...
<?php
		if( isset($prodcat) )
		{
			$q = "SELECT `title` FROM `principals` WHERE `id`={$prodcat} LIMIT 1;";
		
			$rs = mysql_query($q) or die(mysql_error() . $q);
			$r = mysql_fetch_assoc($rs);
			$prodcat_title = $r['title'];
		}	
		else
...
<?php
	...
	if (!isset($id)) { //if 1 
?>
<FORM action='dr.php?prodcat=<?php echo $prodcat ?>' method='post'>
		...
		<TR>
			<TD rowspan='2' valign='top'>
				...
				<SELECT id='accttype' name='accttype' style='width:240' TABINDEX="7" >
					...
					<OPTION value=8>** Credit Memo/Warehouse BO</OPTION>
					<OPTION value=9>** Panel BO Report</OPTION>
					<OPTION value=10>** Panel Inventory Report</OPTION>
					<!--<OPTION value=11>DSR</OPTION>-->
			  </SELECT>
			</TD>
...
?>
<FORM ACTION="dr.php?prodcat=<?php echo $prodcat ?>&id=<?php echo $id ?>" METHOD="post">
  ...
  <!--DWLayoutTable-->
	...
	<TR height='24'>
		...
		<TD COLSPAN="2" align='right' VALIGN="middle" BGCOLOR="#CCCCCC">
			<A HREF="print<?php if( $accttype < 8 ) { echo "dr"; } else { echo "inv"; } ?>.php?prodcat=<?php echo $prodcat ?>&id=<?php echo $id ?>&xls=true">
			<img src="images/printer.gif" width="34" height="34" border="0"></A><A HREF="dr.php?prodcat=<?php echo $prodcat ?>&id=<?php echo $id ?>"><IMG SRC="images/refresh_v.JPG" WIDTH="16" HEIGHT="16" BORDER="0"></A></TD>
	</TR>
	<TR height='24' CLASS="even">
		...
		<TD align='right' VALIGN="top"><SELECT select style='width:100%' name='selectUnit'  TABINDEX="3">
			<OPTION>bdl.</OPTION>
			<OPTION>pcs.</OPTION>
        </SELECT>		  </TD>
		<TD COLSPAN="2" align='right' VALIGN="top">
		<INPUT type='submit' name='Submit' value='+'  TABINDEX="4">
		</TD>
	</TR>
	<TR height='24' CLASS="even">
	...
	{	//add
		...
		if ($unit == "pcs.") {
			//todododododo
			$resn = mysql_query("SELECT `BQ` FROM `varsizes` WHERE `ID`=$vsid") or die(mysql_error());
			$fres = mysql_fetch_assoc($resn);
			$bq = $fres['BQ'];
			$amount = ($qty / $bq)* $unitprice;
			$netamount = ($qty / $bq) * $netprice;
			$unitprice = $unitprice / $bq;
			$netprice = $netprice / $bq;
	...
	<a href='dr.php?prodcat=$prodcat&id=$id&delitem=$p_id'>X</a></td>
