...
<FORM ACTION="prices.php" METHOD="post">
        <TABLE WIDTH="746" BORDER="0" CELLPADDING="1" CELLSPACING="0">
          <!--DWLayoutTable-->
          <TR>
            <TD HEIGHT="25" COLSPAN="7" VALIGN="top">
			<STRONG>
				Manual Price List [
				<A href='prices2.php?prodcat=1'>
				<?php if( $prodcat==1 ) { echo "<strong>"; } ?>
				Oishi
				<?php if( $prodcat==1 ) { echo "</strong>"; } ?>
				</A> 
				| 
				<A href='prices2.php?prodcat=2'>
				<?php if( $prodcat==2 ) { echo "<strong>"; } ?>
				Oishi 2
				<?php if( $prodcat==2 ) { echo "</strong>"; } ?>
				</A> ]
			</STRONG>
		  ...
		  <TR>
			<TD WIDTH="273" HEIGHT="25" VALIGN="middle">Product/ SKU Name </TD>
		  ...
		  <?php
				...
				$query_articles_join = 
					...
					"
						SELECT
							...
							ON
								VARSIZES.PVarID = PRODUCTVARS.ID							
						ORDER BY ProdGroup ASC, ProductName ASC, VariantName ASC, xVS ASC;
					", $prodcat);			
			}
			...
			{			
				...
				$xprice_vatbkg = number_format($rowArticles['VATBKG'], 2, '.', '');
				$xprice_vatpbkg = number_format($rowArticles['VATPBKG'], 2, '.', '');
				$xprodgroup = $rowArticles['ProdGroup'];
				$xbq = $rowArticles['BQ'];
	/*				
	...
	//*/			
				...
				echo "<tr class=$cl>
						...
						<td align='right' valign='middle'>
							$zerotag[0] $xprice_panel
						</td>
						<td align='right' valign='middle'>
							$zerotag[1] $xprice_booking
						</td>
						<td align='right' valign='middle'>
							$zerotag[2] $xprice_pbooking
						</td>
						<td align='right' valign='middle'>
							$zerotag[3] $xprice_landed
						</td>
						<td align='right' valign='middle'>
							$zerotag[4] $xprice_vatbkg
						</td>
						<td align='right' valign='middle'>
							$zerotag[5] $xprice_vatpbkg $xprodgroup
						</td>
						<td align='right' valign='middle'>
							$zerotag[6] $xbq 
						</td>
